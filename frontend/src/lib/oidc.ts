...
/**
 * Cliente OIDC do lado do SERVIDOR (copia do portal, ver infra/portal/lib).
 *
...
export function clientId(): string {
  return process.env.OIDC_CLIENT_ID ?? 'despesa-web'
}
...
/**
 * URL publica DESTE app, montada do HOST_IP — NUNCA de `req.url`.
 *
 * Atencao a porta: o container escuta na 3000 (PORT), mas e publicado na 3010,
 * que e a porta registrada no redirect URI do realm. Usar PORT aqui montaria
 * `http://<ip>:3000/...`, que e o frontend do INVENTARIO — o Keycloak recusaria
 * o redirect_uri e o login falharia com "Invalid parameter".
 *
...
export function baseUrl(): string {
  return `http://${obrigatoria('HOST_IP')}:${process.env.PORTA_PUBLICA ?? '3010'}`
}
